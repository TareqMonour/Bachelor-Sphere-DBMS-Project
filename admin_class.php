...
Class Action {
    ...
    function save_mess(){
        ...
        if(empty($id)){
            $save = $this->db->query("INSERT INTO mess_management set $data");
        } else {
            $save = $this->db->query("UPDATE mess_management set $data where id = $id");
        }
        if($save)
            return 1;
    }
    ...
    function delete_mess(){
        extract($_POST);
        $delete = $this->db->query("DELETE FROM mess_management WHERE id = $id");
        if($delete) {
            return 1;
        }
        return 0;
    ...
    function save_rickshaw_fare(){
        extract($_POST);
        $data = " origin = '$origin' ";
        $data .= ", destination = '$destination' ";
        $data .= ", fare = '$fare' ";
        if(empty($id)){
            $save = $this->db->query("INSERT INTO rickshaw_fares set $data");
        } else {
            $save = $this->db->query("UPDATE rickshaw_fares set $data where id = $id");
        }
        if($save)
            return 1;
    }

    function delete_rickshaw_fare(){
        extract($_POST);
        $delete = $this->db->query("DELETE FROM rickshaw_fares WHERE id = $id");
        if($delete) {
            return 1;
        }
        return 0;
   ...
   function save_lost_found(){
    extract($_POST);
    $data = " item_name = '$item_name' ";
    $data .= ", description = '$description' ";
    $data .= ", location = '$location' ";
    if(empty($id)){
        $save = $this->db->query("INSERT INTO lost_and_found SET $data");
    } else {
        $save = $this->db->query("UPDATE lost_and_found SET $data WHERE id = $id");
    }
    if($save){
        return 1;
    } else {
        return "Error: " . $this->db->error; // Return the error message
    }
}


    function delete_lost_found(){
        extract($_POST);
        $delete = $this->db->query("DELETE FROM lost_and_found WHERE id = $id");
        if($delete) {
            return 1;
        }
        return 0;
    ...
    function save_tuition(){
        extract($_POST);
        $data = " student_name = '$student_name' ";
        $data .= ", course = '$course' ";
        $data .= ", amount = '$amount' ";
        if(empty($id)){
            $save = $this->db->query("INSERT INTO tuition set $data");
        } else {
            $save = $this->db->query("UPDATE tuition set $data where id = $id");
        }
        if($save)
            return 1;
    }

    function delete_tuition(){
        extract($_POST);
        $delete = $this->db->query("DELETE FROM tuition WHERE id = $id");
        if($delete) {
            return 1;
        }
        return 0;
